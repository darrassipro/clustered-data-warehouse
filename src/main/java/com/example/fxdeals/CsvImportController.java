    ...
    public Map<String, Object> importCsv(@RequestParam("file") MultipartFile file) {
        ...
        try (CSVReader reader = new CSVReader(new InputStreamReader(file.getInputStream()))) {
            ...
            while ((line = reader.readNext()) != null) {
                ...
                try {
                    // 1. Create and Populate DTO
                    DealDto dealDto = new DealDto();
                    dealDto.setDealUniqueId(line[0]);
                    dealDto.setOrderingCurrency(line[1]);
                    dealDto.setDeliveryCurrency(line[2]);
                    dealDto.setDealTimestamp(line[3]);
                    dealDto.setAmount(Double.parseDouble(line[4]));

                    // 2. Convert DTO to Entity
                    Deal deal = dealDto.toEntity();

                    // 3. Save Entity
                    boolean savedOk = service.saveIfNotExists(deal);

                    if (savedOk) saved++;
                    else errors.add("Duplicate deal: " + dealDto.getDealUniqueId());
